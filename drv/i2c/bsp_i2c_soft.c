...
{
    HAL_GPIO_WritePin(IIC_SCL_GPIO_PORT, IIC_SCL_GPIO_PIN, (GPIO_PinState)ucData);
    return 0;
}

static char I2C_Sda(unsigned char ucData)
{
    HAL_GPIO_WritePin(IIC_SDA_GPIO_PORT, IIC_SDA_GPIO_PIN, (GPIO_PinState)ucData);
    return 0;
}

static char I2C_SdaRead(void)
{
    return HAL_GPIO_ReadPin(IIC_SDA_GPIO_PORT, IIC_SDA_GPIO_PIN);
}
...
void I2C2_Init(void)
{
    ...
    // 地址添加
    I2C_InterfaceResister(&gstRtcI2c, , I2C2_Scl, I2C2_Sda, I2C2_SdaRead, I2C_Delay);
    I2C_Stop(&gstRtcI2c);
}
 ...
 * @author: lkc
 */
bool SOFT_I2C_Mem_Write(void *i2c, uint16_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, unsigned char *pData,
 ...
 * @author: lkc
 */
bool SOFT_I2C_Mem_Read(void *i2c, uint16_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, unsigned char *pData,
                       uint16_t Size, uint64_t timeout)
{
    unsigned char ack;
