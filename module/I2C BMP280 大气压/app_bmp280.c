...
static void BMP280GetPressure(void)
{
    ...
    bmp280RawTemperature =
        (int32_t)((((uint32_t)(data[3])) << 12) | (((uint32_t)(data[4])) << 4) | ((uint32_t)data[5] >> 4));
}
...
static int32_t BMP280CompensateT(int32_t adcT)
{
    ...
    var2 = (((((adcT >> 4) - ((int32_t)bmp280Cal.dig_T1)) * ((adcT >> 4) - ((int32_t)bmp280Cal.dig_T1))) >> 12) *
            ((int32_t)bmp280Cal.dig_T3)) >>
           14;
...
#define FIX_TEMP                                                                                                       \
    25 // Fixed Temperature. ASL is a function of pressure and temperature, but as the temperature changes so much (blow
       // a little towards the flie and watch it drop 5 degrees) it corrupts the ASL estimates. TLDR: Adjusting for temp
       // changes does more harm than good.
/**
