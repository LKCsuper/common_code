// 未验证
/*
 * Jan 22, Dortain modifies function for CRC32 of 32 bit-wise
 *
...
const uint32 crc32table_le[LE_TABLE_ROWS][256] = {
...
static void crc32init_be_generic(const uint32 polynomial, uint32 (*tab)[256])
{
    uint32 i, j;
...
/* implements slicing-by-4 algorithm */
static inline uint32 crc32_body(uint32 crc, const uint8 *buf, uint32 len, const uint32 (*tab)[256])
{
#ifdef LITTLE_ENDIAN
...
#define DO_CRC4   (t0[(q) & 255] ^ t1[(q >> 8) & 255] ^ t2[(q >> 16) & 255] ^ t3[(q >> 24) & 255])
    ...
    /* Make sure that the buf addr is multiples of 4 */
    if (((uint32)buf & 3) && len) {
        do {
            DO_CRC(*buf++);
        } while ((--len) && (((uint32)buf) & 3));
    }
 ...
 */
static inline uint32 crc32_le_generic(uint32 crc, const uint8 *p, uint32 len, const uint32 (*tab)[256],
                                      uint32 polynomial)
{
 ...
 */
#ifndef LITTLE_ENDIAN
static inline uint32 crc32_be_generic(uint32 crc, const uint8 *p, uint32 len, uint32 (*tab)[256], uint32 polynomial)
{
    crc = crc32_body(crc, p, len, tab);
    return crc;
}
...
{
    uint32 crc32table_be[BE_TABLE_ROWS][256];
