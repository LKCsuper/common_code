/*
 * @Description: 已验证 CRC校验
 * @Version: 1.0
 * @Author: lkc
 * @Date: 2023-11-15 15:09:25
 * @LastEditors: lkc
 * @LastEditTime: 2023-11-15 15:22:47
 * @detail:
 */
#ifdef __cplusplus
extern "C" {
#endif
/* Includes ------------------------------------------------------------------*/
#include "common.h"
/* Private define ------------------------------------------------------------*/
/* Private typedef -----------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
const uint8_t constCRC16_Hi[] = {
    };
/* CRC低位字节值表*/
const uint8_t constCRC16_Lo[] = {
    };
/* Private function prototypes -----------------------------------------------*/
/* Private functions ---------------------------------------------------------*/
/**
 * @description: 计算纵向冗余校验
 * @detail:
 * @param {uint8_t} *string
 * @param {uint16_t} length
 * @return {*}
 * @author: lkc
 */
uint8_t Common_BccCal(uint8_t *string, uint16_t length)
{
    uint8_t bcc = 0;
    uint16_t i;

    bcc = *string;
    for (i = 1; i < length; i++) {
        bcc ^= *(string + i);
    }

    return bcc;
}

/**
 * @description: CRC16 modbus
 * @detail:
 * @param {uint8_t} *data
 * @param {uint16_t} length
 * @return {*}
 * @author: lkc
 */
uint16_t Common_CRC16(uint8_t *data, uint16_t length)
{
    uint8_t CRCHi = 0xff; // 高CRC字节初始化
    uint8_t CRCLo = 0xff; // 低CRC 字节初始化
    unsigned long index;  // CRC循环中的索引
    uint16_t len = length;

    while (len--) // 传输消息缓冲区
    {
        index = CRCHi ^ *data++; // 计算CRC
        CRCHi = CRCLo ^ constCRC16_Hi[index];
        CRCLo = constCRC16_Lo[index];
    }
    return (CRCHi << 8 | CRCLo);
}
#ifdef __cplusplus
}
#endif
