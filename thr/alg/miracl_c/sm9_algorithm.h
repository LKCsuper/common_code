...
//@x sm9加密主公钥的x分量
//@y sm9加密主公钥的y分量
//@cGtchar 初始化SM9需要用到的预计算结果
// 返回值 预计算结果
// 第一次调用gc = SM9_Set_Encap(x,y,NULL)，以后可以调用SM9_Set_Encap(NULL,NULL,gc)完成初始化
// 可将gc写入文件或数据库，以后从文件或数据库中读取
...
// 根据安全参数secLevel初始化一个封装数据结构体，标准为32
...
// 释放一个封装结构体
void SM9_Package_Free(SM9_Package *p);
...
// 0 生成成功
// 1 系统未初始化
int SM9_GenMEncapPubKey(SM9_MSK *msk, SM9_MCPK *mcpk);
...
// 2 G1生成元配置错误
// 3 G2生成元配置错误
int SM9_Decrypt(SM9_PK *pk, unsigned int KDF_ID, SM9_CSK *sk, SM9_Cipher *cip, unsigned char *mes,
                unsigned int *meslen);
...
//@pk       the public key(id) for the receiver
//@pPack    the packtext
//@iKeyLen  the length of the pKey
...
//=============For ReceiveStep algorithm=================
//@ran      the random number used in sendstep (must be 32 char, if it's length less than 32 , plese add 0x00 at the
//first)
...
// 2 G1生成元配置错误
// 3 G2生成元配置错误
int SM9_ReceiveStep(unsigned char *ran, SM9_Send *S, SM9_Send *R, SM9_PK *SP, SM9_PK *RP, SM9_KSK *ksk,
                    unsigned int sklen, unsigned char *S1, unsigned char *S2, unsigned char *SK, unsigned int SorR);
...
// int SM9_KeyExchange_Init(unsigned char *masterPubKey, unsigned char *userId, unsigned char *sr, unsigned char *sR);
