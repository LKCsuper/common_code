...
void zzn12_pow(_MIPD_ zzn12 *x, big e)
{
    ...
    if (nb > 1)
        for (i = nb - 2; i >= 0;) {
            ...
            for (j = 0; j < nbw; j++)
                zzn12_mul(_MIPP_ &u, &u, &u);
            if (n > 0)
                zzn12_mul(_MIPP_ &u, &(t[n / 2]), &u);
            i -= nbw;
            if (nzs) {
                for (j = 0; j < nzs; j++)
                    zzn12_mul(_MIPP_ &u, &u, &u);
...
{
    if (zzn4_compare(&(w->a), &(x->a)) &&zzn4_compare(&(w->b), &(x->b)) &&zzn4_compare(&(w->c), &(x->c))) {
        return TRUE;
    }
    return FALSE;
}
