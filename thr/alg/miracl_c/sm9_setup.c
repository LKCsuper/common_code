...
char ct[32] = { "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x60"
                "\x00\x00\x00\x00\x58\xF9\x8A" };

char cq[32] = { "\xB6\x40\x00\x00\x02\xA3\xA6\xF1\xD6\x03\xAB\x4F\xF5\x8E\xC7\x45\x21\xF2\x93\x4B\x1A\x7A\xEE\xDB\xE5"
                "\x6F\x9B\x27\xE3\x51\x45\x7D" };

char ca[32] = { "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                "\x00\x00\x00\x00\x00\x00\x00" };

char cb[32] = { "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                "\x00\x00\x00\x00\x00\x00\x05" };

char cn[32] = { "\xB6\x40\x00\x00\x02\xA3\xA6\xF1\xD6\x03\xAB\x4F\xF5\x8E\xC7\x44\x49\xF2\x93\x4B\x18\xEA\x8B\xEE\xE5"
                "\x6E\xE1\x9C\xD6\x9E\xCF\x25" };

char cxp1[32] = { "\x93\xDE\x05\x1D\x62\xBF\x71\x8F\xF5\xED\x07\x04\x48\x7D\x01\xD6\xE1\xE4\x08\x69\x09\xDC\x32\x80\xE8"
                  "\xC4\xE4\x81\x7C\x66\xDD\xDD" };

char cyp1[32] = { "\x21\xFE\x8D\xDA\x4F\x21\xE6\x07\x63\x10\x65\x12\x5C\x39\x5B\xBC\x1C\x1C\x00\xCB\xFA\x60\x24\x35\x0C"
                  "\x46\x4C\xD7\x0A\x3E\xA6\x16" };

char cxq1[32] = { "\x37\x22\x75\x52\x92\x13\x0B\x08\xD2\xAA\xB9\x7F\xD3\x4E\xC1\x20\xEE\x26\x59\x48\xD1\x9C\x17\xAB\xF9"
                  "\xB7\x21\x3B\xAF\x82\xD6\x5B" };

char cxq2[32] = { "\x85\xAE\xF3\xD0\x78\x64\x0C\x98\x59\x7B\x60\x27\xB4\x41\xA0\x1F\xF1\xDD\x2C\x19\x0F\x5E\x93\xC4\x54"
                  "\x80\x6C\x11\xD8\x80\x61\x41" };

char cyq1[32] = { "\xA7\xCF\x28\xD5\x19\xBE\x3D\xA6\x5F\x31\x70\x15\x3D\x27\x8F\xF2\x47\xEF\xBA\x98\xA7\x1A\x08\x11\x62"
                  "\x15\xBB\xA5\xC9\x99\xA7\xC7" };

char cyq2[32] = { "\x17\x50\x9B\x09\x2E\x84\x5C\x12\x66\xBA\x0D\x26\x2C\xBE\xE6\xED\x07\x36\xA9\x6F\xA3\x47\xC8\xBD\x85"
                  "\x6D\xC7\x6B\x84\xEB\xEB\x96" };

char cmasterkey[32] = { "\x00\x01\x30\xE7\x84\x59\xD7\x85\x45\xCB\x54\xC5\x87\xE0\x2C\xF4\x80\xCE\x0B\x66\x34\x0F\x31"
                        "\x9F\x34\x8A\x1D\x5B\x1F\x2D\xC5\xF4" };

char sm9Xx[32] = { "\x3f\x23\xea\x58\xe5\x72\x0b\xdb\x84\x3c\x6c\xfa\x9c\x08\x67\x49\x47\xc5\xc8\x6e\x0d\xdd\x04\xed"
                   "\xa9\x1d\x83\x54\x37\x7b\x69\x8b" };
//================================
             ...
             unsigned char *xq1, unsigned char *xq2, unsigned char *yq1, unsigned char *yq2)
{
    ...
    sm9Ctx->sm9t_Ex = mirvar(_MIPP_ 0);

    zzn2_mirvar(_MIPP_ &(sm9Ctx->sm9X_Ex));
    sm9Ctx->p1G1_Ex = epoint_init(_MIPPO_);
    sm9Ctx->ppG1_Ex = epoint_init(_MIPPO_);
    sm9Ctx->keG1_Ex = epoint_init(_MIPPO_);
    sm9Ctx->cpG1_Ex = epoint_init(_MIPPO_);

    ecn2_mirvar(_MIPP_ &(sm9Ctx->p2G2_Ex));
    ecn2_mirvar(_MIPP_ &(sm9Ctx->ppG2_Ex));

    zzn12_mirvar(_MIPP_ &(sm9Ctx->gGt_Ex));
    zzn12_mirvar(_MIPP_ &(sm9Ctx->eGt_Ex));
    zzn12_mirvar(_MIPP_ &(sm9Ctx->kGt_Ex));
    zzn12_mirvar(_MIPP_ &(sm9Ctx->cGt_Ex));
    ...
    if (curve) {
        sm9len = seclevel;
        TWIST  = TWIST_TYPE;
        bytes_to_big(_MIPP_ sm9len, (char *)t, sm9Ctx->sm9t_Ex);
        ...
        if (!epoint_set(_MIPP_ x, y, 1, sm9Ctx->p1G1_Ex)) {
            mirkill(sm9q);
            mirkill(sm9a);
            mirkill(sm9b);
            mirkill(sm9n);
            mirkill(sm9Ctx->sm9t_Ex);

            zzn2_kill(_MIPP_ &(sm9Ctx->sm9X_Ex));
            epoint_free(sm9Ctx->p1G1_Ex);
            epoint_free(sm9Ctx->ppG1_Ex);
            epoint_free(sm9Ctx->keG1_Ex);
            epoint_free(sm9Ctx->cpG1_Ex);
            ecn2_kill(_MIPP_ &(sm9Ctx->p2G2_Ex));
            ecn2_kill(_MIPP_ &(sm9Ctx->ppG2_Ex));
            zzn12_kill(_MIPP_ &(sm9Ctx->gGt_Ex));
            zzn12_kill(_MIPP_ &(sm9Ctx->eGt_Ex));
            zzn12_kill(_MIPP_ &(sm9Ctx->kGt_Ex));
            zzn12_kill(_MIPP_ &(sm9Ctx->cGt_Ex));
        ...
        if (!ecn2_set(_MIPP_ &xx, &yy, &(sm9Ctx->p2G2_Ex))) {
            mirkill(sm9q);
            mirkill(sm9a);
            mirkill(sm9b);
            mirkill(sm9n);
        ...
        set_frobenius_constant(_MIPP_ &(sm9Ctx->sm9X_Ex));
    } else {
        sm9len = 32;
        TWIST  = twist;
        bytes_to_big(_MIPP_ sm9len, (char *)ct, sm9Ctx->sm9t_Ex);
        ...
        zzn2_from_big(_MIPP_ x, &(sm9Ctx->sm9X_Ex));
    }
    zzn2_kill(_MIPP_ &xx);
    zzn2_kill(_MIPP_ &yy);
    mirkill(x);
...
unsigned char *SM9_Set_Sign(unsigned char *x1, unsigned char *x2, unsigned char *y1, unsigned char *y2,
                            unsigned char *gGtchar)
{
    miracl *mr_mip;
    big x, y;
    zzn2 xx, yy;
    ...
    copy(sm9Ctx->sm9t_Ex, mirCtx->mcs_sm9t);
    zzn2_copy(&(sm9Ctx->sm9X_Ex), &(mirCtx->mcs_sm9X));
    epoint_copy(sm9Ctx->p1G1_Ex, mirCtx->mcs_p1G1);
    ecn2_copy(&(sm9Ctx->p2G2_Ex), &(mirCtx->mcs_p2G2));
    ecn2_copy(&(sm9Ctx->ppG2_Ex), &(mirCtx->mcs_ppG2));
    zzn12_copy(&(sm9Ctx->gGt_Ex), &(mirCtx->mcs_gGt));
...
unsigned char *SM9_Set_Encrypt(unsigned char *x, unsigned char *y, unsigned char *eGtchar)
{
    ...
    zzn2_copy(&(sm9Ctx->sm9X_Ex), &(mirCtx->mcs_sm9X));
    epoint_copy(sm9Ctx->p1G1_Ex, mirCtx->mcs_p1G1);
    epoint_copy(sm9Ctx->ppG1_Ex, mirCtx->mcs_ppG1);
    ecn2_copy(&(sm9Ctx->p2G2_Ex), &(mirCtx->mcs_p2G2));
    zzn12_copy(&(sm9Ctx->eGt_Ex), &(mirCtx->mcs_eGt));
...
unsigned char *SM9_Set_Encap(unsigned char *x, unsigned char *y, unsigned char *cGtchar)
{
    miracl *mr_mip;
    big a, b;
    unsigned char *gc;
    ...
    zzn2_copy(&(sm9Ctx->sm9X_Ex), &(mirCtx->mcs_sm9X));
    epoint_copy(sm9Ctx->p1G1_Ex, mirCtx->mcs_p1G1);
    epoint_copy(sm9Ctx->cpG1_Ex, mirCtx->mcs_cpG1);
    ecn2_copy(&(sm9Ctx->p2G2_Ex), &(mirCtx->mcs_p2G2));
    zzn12_copy(&(sm9Ctx->cGt_Ex), &(mirCtx->mcs_cGt));
    ...
    if (!epoint_set(_MIPP_ a, b, 1, cpG1)) {
        mirkill(a);
        mirkill(b);
        CloseMiracl(mr_mip);
    ...
    if (cGtchar == NULL) {
        sm9encap = ecap(_MIPP_ &p2G2, cpG1, sm9t, &sm9X, &cGt);
        gc         = (unsigned char *)malloc(sizeof(unsigned char) * 12 * sm9len);
        zzn12_tochar(_MIPP_ &cGt, gc, sm9len);
    } else {
        zzn12_fromchar(_MIPP_ &cGt, cGtchar, sm9len);
        sm9encap = TRUE;
        gc         = cGtchar;
    }
    mirkill(a);
...
unsigned char *SM9_Set_KeyExchange(unsigned char *x, unsigned char *y, unsigned char *kGtchar)
{
    ...
    zzn2_copy(&(sm9Ctx->sm9X_Ex), &(mirCtx->mcs_sm9X));
    epoint_copy(sm9Ctx->p1G1_Ex, mirCtx->mcs_p1G1);
    epoint_copy(sm9Ctx->keG1_Ex, mirCtx->mcs_keG1);
    ecn2_copy(&(sm9Ctx->p2G2_Ex), &(mirCtx->mcs_p2G2));
    zzn12_copy(&(sm9Ctx->kGt_Ex), &(mirCtx->mcs_kGt));
...
SM9_Package SM9_Package_New(int secLevel)
{
    SM9_Package pack;
    pack.secLevel = secLevel;
    pack.x        = (unsigned char *)malloc(sizeof(char) * secLevel);
    pack.y        = (unsigned char *)malloc(sizeof(char) * secLevel);
    return pack;
}
...
void SM9_Package_Free(SM9_Package *pack)
{
    free(pack->x);
    free(pack->y);
}
...
BOOL SM9_GenMEncapPubKey(SM9_MSK *msk, SM9_MCPK *mcpk)
{
    miracl *mr_mip;
    big mk;
    if (!sm9init) {
    ...
    ecurve_mult(_MIPP_ mk, p1G1, cpG1);
    epoint_norm(_MIPP_ cpG1);

    redc(_MIPP_ cpG1->X, mk);
    big_to_bytes(_MIPP_ mcpk->secLevel, mk, (char *)mcpk->x, TRUE);
    redc(_MIPP_ cpG1->Y, mk);
    big_to_bytes(_MIPP_ mcpk->secLevel, mk, (char *)mcpk->y, TRUE);
