...
#define NONLINEAR_CHANGE(a)                                                                                            \
    ((SBOX[(a) >> 24] << 24) | (SBOX[((a) >> 16) & 0xFF] << 16) | (SBOX[((a) >> 8) & 0xFF] << 8) | (SBOX[(a)&0xFF]))
...
#define ByteSub(_A)                                                                                                    \
    (Sbox[((unsigned long)_A) >> 24 & 0xFF] << 24 ^ Sbox[((unsigned long)_A) >> 16 & 0xFF] << 16 ^                     \
     Sbox[((unsigned long)_A) >> 8 & 0xFF] << 8 ^ Sbox[((unsigned long)_A) & 0xFF])
...
const U32 CK[32] = { 0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
...
void sm4_set_key(const U8 *userkey, U32 *round_key)
{
    ...
    round_key[10] =
      round_key[6] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[7] ^ round_key[8] ^ round_key[9] ^ CK[10]));
    round_key[11] =
      round_key[7] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[8] ^ round_key[9] ^ round_key[10] ^ CK[11]));
    round_key[12] =
      round_key[8] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[9] ^ round_key[10] ^ round_key[11] ^ CK[12]));
    round_key[13] =
      round_key[9] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[10] ^ round_key[11] ^ round_key[12] ^ CK[13]));
    round_key[14] =
      round_key[10] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[11] ^ round_key[12] ^ round_key[13] ^ CK[14]));
    round_key[15] =
      round_key[11] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[12] ^ round_key[13] ^ round_key[14] ^ CK[15]));
    round_key[16] =
      round_key[12] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[13] ^ round_key[14] ^ round_key[15] ^ CK[16]));
    round_key[17] =
      round_key[13] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[14] ^ round_key[15] ^ round_key[16] ^ CK[17]));
    round_key[18] =
      round_key[14] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[15] ^ round_key[16] ^ round_key[17] ^ CK[18]));
    round_key[19] =
      round_key[15] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[16] ^ round_key[17] ^ round_key[18] ^ CK[19]));
    round_key[20] =
      round_key[16] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[17] ^ round_key[18] ^ round_key[19] ^ CK[20]));
    round_key[21] =
      round_key[17] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[18] ^ round_key[19] ^ round_key[20] ^ CK[21]));
    round_key[22] =
      round_key[18] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[19] ^ round_key[20] ^ round_key[21] ^ CK[22]));
    round_key[23] =
      round_key[19] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[20] ^ round_key[21] ^ round_key[22] ^ CK[23]));
    round_key[24] =
      round_key[20] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[21] ^ round_key[22] ^ round_key[23] ^ CK[24]));
    round_key[25] =
      round_key[21] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[22] ^ round_key[23] ^ round_key[24] ^ CK[25]));
    round_key[26] =
      round_key[22] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[23] ^ round_key[24] ^ round_key[25] ^ CK[26]));
    round_key[27] =
      round_key[23] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[24] ^ round_key[25] ^ round_key[26] ^ CK[27]));
    round_key[28] =
      round_key[24] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[25] ^ round_key[26] ^ round_key[27] ^ CK[28]));
    round_key[29] =
      round_key[25] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[26] ^ round_key[27] ^ round_key[28] ^ CK[29]));
    round_key[30] =
      round_key[26] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[27] ^ round_key[28] ^ round_key[29] ^ CK[30]));
    round_key[31] =
      round_key[27] ^ LINE_CHANGE_L1(NONLINEAR_CHANGE(round_key[28] ^ round_key[29] ^ round_key[30] ^ CK[31]));
...
void SM4CBCEnc(unsigned char *ivec, unsigned char *cin, unsigned int len, unsigned int *key, unsigned char *out)
{
    ...
    if (1 && ((unsigned long)cin | (unsigned long)out | (unsigned long)ivec) % sizeof(unsigned long) != 0) {
        while (len >= 16) {
            for (n = 0; n < 16; ++n)
                out[n] = cin[n] ^ iv[n];
        ...
        while (len >= 16) {
            for (n = 0; n < 16; n += sizeof(unsigned int))
                *(unsigned int *)(out + n) = *(unsigned int *)(cin + n) ^ *(unsigned int *)(iv + n);
            SM4Crypt(out, out, key);
            iv = out;
            len -= 16;
            cin += 16;
        ...
        if (1 && ((unsigned long)cin | (unsigned long)out | (unsigned long)ivec) % sizeof(unsigned long) != 0) {
            ...
            while (len >= 16) {
                ...
                for (n = 0; n < 16; n += sizeof(unsigned int)) {
                    ...
                    *(unsigned int *)(out + n)  = *(unsigned int *)(tmp.c + n) ^ *(unsigned int *)(ivec + n);
