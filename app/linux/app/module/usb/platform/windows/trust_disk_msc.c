...
static int MassStorageInquiry(HANDLE fileHandle, unsigned char *buf)
{
    ...
    UCHAR dirFlag = SCSI_IOCTL_DATA_IN;
    vcmd_t vcmd;

    memset(&vcmd, 0x00, sizeof(vcmd));
    vcmd.rsvd = 0x12;
    vcmd.detail[0] = 36;
    ...
    sptdwb.sptd.DataIn = dirFlag;
    sptdwb.sptd.SenseInfoLength = 24;
    sptdwb.sptd.DataTransferLength = 36; // note!
    sptdwb.sptd.TimeOutValue = 5;
    sptdwb.sptd.DataBuffer = buf;
    sptdwb.sptd.SenseInfoOffset = offsetof(SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER, ucSenseBuf);
    memcpy(sptdwb.sptd.Cdb, &vcmd, 16);
    length = sizeof(SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER);

    if (!DeviceIoControl(fileHandle, IOCTL_SCSI_PASS_THROUGH_DIRECT, &sptdwb, length, &sptdwb, length, &bytesRet, NULL)) {
        // printf("vcmd 0x%x issue fail, errno=%d\n", vcmd.op, GetLastError());
        return 0;
    } else {
        if (sptdwb.sptd.ScsiStatus > 0) {
            // printf("vcmd 0x%x exec fail, SCSI Statuss = %d\n", vcmd.op, sptdwb.sptd.ScsiStatus);
            return 0;
        } else {
            return 1;
        }
